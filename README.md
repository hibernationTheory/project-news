...
This is a simplified fork of the [food-lookup-demo](https://github.com/fullstackreact/food-lookup-demo). It demonstrates using the setup generated by `create-react-app` alongside a Node Express API server. 

The simplified version gets rid of the client side dependencies and most of the functionality to make things easier to understand. Client side is there to ensure the server-client communication works and once that is established it can be removed by the desired client-side react.js app.

The server side is also simplified with the removal of the sqlite db.
...
```
git clone <address> target_folder
cd target_folder
npm i
